...
def graphical_output(data,p,path_to_temp,path_to_output):
    ...
    for mode in p["simulation"]["output"]["graphical"]:
        if mode == "raster":
            for k in data.keys():
                fig = raster(data,k)
                plt.savefig(path_to_output + "/Rasters_{}.tif".format(k))
                plt.close(fig)
                
        if mode == "mean_currents":
            for k in data.keys():
                fig = mean_current_lines(data,k)
                plt.savefig(path_to_output + "/Mean_clines_{}.tif".format(k))
                plt.close(fig)
    
    
def output_data(p,layers,recorders,path_to_temp,path_to_output):
    ...
    for rec in recorders:
        ...
        if not temp["senders"].any():
            temp = af.read_data(rec,p,path_to_temp,path_to_output)  
        ...
        if ".mat" in t["formats"]:
            sp.savemat("{}/{}.mat".format(path_to_output,name),{"data":full_data})
            
        graphical_output(full_data,p,path_to_temp,path_to_output)

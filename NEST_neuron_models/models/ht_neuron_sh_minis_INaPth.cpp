...
#include "ht_neuron_sh_minis_INaPth.h"
...
namespace nest
{
RecordablesMap< ht_neuron_sh_minis_INaPth > ht_neuron_sh_minis_INaPth::recordablesMap_;
template <>
void
RecordablesMap< ht_neuron_sh_minis_INaPth >::create()
{
  insert_( names::V_m, &ht_neuron_sh_minis_INaPth::get_y_elem_< ht_neuron_sh_minis_INaPth::State_::V_M > );
  insert_( names::theta, &ht_neuron_sh_minis_INaPth::get_y_elem_< ht_neuron_sh_minis_INaPth::State_::THETA > );
  insert_(
    names::g_AMPA, &ht_neuron_sh_minis_INaPth::get_y_elem_< ht_neuron_sh_minis_INaPth::State_::G_AMPA > );
  insert_( names::g_NMDA, &ht_neuron_sh_minis_INaPth::get_g_NMDA_ );
  insert_(
    names::g_GABA_A, &ht_neuron_sh_minis_INaPth::get_y_elem_< ht_neuron_sh_minis_INaPth::State_::G_GABA_A > );
  insert_(
    names::g_GABA_B, &ht_neuron_sh_minis_INaPth::get_y_elem_< ht_neuron_sh_minis_INaPth::State_::G_GABA_B > );
  insert_( names::I_NaP, &ht_neuron_sh_minis_INaPth::get_I_NaP_ );
  insert_( names::I_KNa, &ht_neuron_sh_minis_INaPth::get_I_KNa_ );
  insert_( names::I_T, &ht_neuron_sh_minis_INaPth::get_I_T_ );
  insert_( names::I_h, &ht_neuron_sh_minis_INaPth::get_I_h_ );
}
...
extern "C" inline int
ht_neuron_sh_minis_INaPth_dynamics( double, const double y[], double f[], void* pnode )
{
  // shorthand
  typedef nest::ht_neuron_sh_minis_INaPth::State_ S;

  // get access to node so we can almost work as in a member class
  assert( pnode );
  nest::ht_neuron_sh_minis_INaPth& node = *( reinterpret_cast< nest::ht_neuron_sh_minis_INaPth* >( pnode ) );
  ...
  // intrinsic currents
  // I_Na(p), m_inf^1; ^3 according to Compte et al, J Neurophysiol 2003 89:2707 but ^1 restored for consistency with the original Synthesis code
  //const double INaP_thresh = -55.7;
  const double INaP_slope = 7.7;
  const double m_inf_NaP =
    1.0 / ( 1.0 + std::exp( -( V - node.P_.INaP_thresh ) / INaP_slope ) );
  node.S_.I_NaP_ = -node.P_.g_peak_NaP * std::pow( m_inf_NaP, 1.0 )
    * ( V - node.P_.E_rev_NaP );
  ...
  // I_T  with m^1 restored for consistency with the original Synthesis code
  node.S_.I_T_ = -node.P_.g_peak_T * y[ S::m_IT ] * y[ S::h_IT ]
    * ( V - node.P_.E_rev_T );
  ...
  // I_KNa with original Synthesis ODE restored
  const double Deq = 0.001;
  f[ S::D_IKNa ] = node.D_eq_KNa_( V ) - ( y[ S::D_IKNa ]*( 1.0 - Deq ) ) 
      / node.P_.tau_D_KNa;
...
inline double
nest::ht_neuron_sh_minis_INaPth::m_eq_h_( double V ) const
{
...
inline double
nest::ht_neuron_sh_minis_INaPth::h_eq_T_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( ( V + 83.0 ) / 4 ) );
}

inline double
nest::ht_neuron_sh_minis_INaPth::m_eq_T_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( -( V + 59.0 ) / 6.2 ) );
}

inline double
nest::ht_neuron_sh_minis_INaPth::D_eq_KNa_( double V ) const
{
  const double D_influx_peak = 0.02;    //0.025
...
inline double
nest::ht_neuron_sh_minis_INaPth::m_eq_NMDA_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( -P_.S_act_NMDA * ( V - P_.V_act_NMDA ) ) );
}

inline double
nest::ht_neuron_sh_minis_INaPth::m_NMDA_( double V,
  double m_eq,
  double m_fast,
  double m_slow ) const
{
...
inline double
nest::ht_neuron_sh_minis_INaPth::get_g_NMDA_() const
{
...
nest::ht_neuron_sh_minis_INaPth::Parameters_::Parameters_()
  : E_Na( 30.0 ) // mV
  , E_K( -90.0 ) // mV
  , g_NaL( 0.2 )
  , g_KL( 1.0 )
...
nest::ht_neuron_sh_minis_INaPth::State_::State_( const ht_neuron_sh_minis_INaPth& node, const Parameters_& p )
  : ref_steps_( 0 )
  , I_NaP_( 0.0 )
  , I_KNa_( 0.0 )
  , I_T_( 0.0 )
...
nest::ht_neuron_sh_minis_INaPth::State_::State_( const State_& s )
...
nest::ht_neuron_sh_minis_INaPth::State_& nest::ht_neuron_sh_minis_INaPth::State_::operator=( const State_& s )
{
  if ( this == &s )
  {
    return *this;
...
nest::ht_neuron_sh_minis_INaPth::State_::~State_()
{
}
...
void
nest::ht_neuron_sh_minis_INaPth::Parameters_::get( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::INaP_thresh, INaP_thresh );
  ...
  def< double >( d, names::mini_mean, mini_mean);
  def< double >( d, names::mini_sigma, mini_sigma);
}

void
nest::ht_neuron_sh_minis_INaPth::Parameters_::set( const DictionaryDatum& d )
{
  ...
  updateValue< double >( d, names::INaP_thresh, INaP_thresh );
  ...
  updateValue< double > ( d, names::mini_mean, mini_mean );
  updateValue< double > ( d, names::mini_sigma, mini_sigma );
...
void
nest::ht_neuron_sh_minis_INaPth::State_::get( DictionaryDatum& d ) const
{
...
void
nest::ht_neuron_sh_minis_INaPth::State_::set( const DictionaryDatum& d, const ht_neuron_sh_minis_INaPth& node )
{
...
nest::ht_neuron_sh_minis_INaPth::Buffers_::Buffers_( ht_neuron_sh_minis_INaPth& n )
  : logger_( n )
...
nest::ht_neuron_sh_minis_INaPth::Buffers_::Buffers_( const Buffers_&, ht_neuron_sh_minis_INaPth& n )
  : logger_( n )
  , spike_inputs_( std::vector< RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) )
  , s_( 0 )
  , c_( 0 )
...
nest::ht_neuron_sh_minis_INaPth::ht_neuron_sh_minis_INaPth()
  : Archiving_Node()
  , P_()
  , S_( *this, P_ )
  , B_( *this )
...
nest::ht_neuron_sh_minis_INaPth::ht_neuron_sh_minis_INaPth( const ht_neuron_sh_minis_INaPth& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::ht_neuron_sh_minis_INaPth::init_state_( const Node& proto )
{
  const ht_neuron_sh_minis_INaPth& pr = downcast< ht_neuron_sh_minis_INaPth >( proto );
  S_ = pr.S_;
}

void
nest::ht_neuron_sh_minis_INaPth::init_buffers_()
{
  // Reset spike buffers.
  ...
  B_.sys_.function = ht_neuron_sh_minis_INaPth_dynamics;
  B_.sys_.jacobian = 0;
...
double
nest::ht_neuron_sh_minis_INaPth::get_synapse_constant( double tau_1,
  double tau_2,
  double g_peak )
{
...
void
nest::ht_neuron_sh_minis_INaPth::calibrate()
{
...
void
nest::ht_neuron_sh_minis_INaPth::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
  ...
  ( *receptor_type )[ names::MINI ] = MINI; //added this
...
void
nest::ht_neuron_sh_minis_INaPth::set_status( const DictionaryDatum& d )
{
...
void
ht_neuron_sh_minis_INaPth::update( Time const& origin, const long from, const long to )
{
  assert(
...
void
nest::ht_neuron_sh_minis_INaPth::handle( SpikeEvent& e )
{
...
void
nest::ht_neuron_sh_minis_INaPth::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::ht_neuron_sh_minis_INaPth::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
}

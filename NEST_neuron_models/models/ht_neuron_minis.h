...
#ifndef ht_neuron_minis_H
#define ht_neuron_minis_H
...
/* BeginDocumentation
   Name: ht_neuron_minis - Neuron model after Hill & Tononi (2005).
   Description:
 ...
 */
extern "C" int ht_neuron_minis_dynamics( double, const double*, double*, void* );

class ht_neuron_minis : public Archiving_Node
{
public:
  ht_neuron_minis();
  ht_neuron_minis( const ht_neuron_minis& );
  ~ht_neuron_minis();
...
private:
  ...
  friend int ht_neuron_minis_dynamics( double, const double*, double*, void* );
...
public:
  struct State_
  {
    ...
    State_( const ht_neuron_minis&, const Parameters_& p );
    ...
    void set( const DictionaryDatum&, const ht_neuron_minis& );
  };

private:
  // These friend declarations must be precisely here.
  friend class RecordablesMap< ht_neuron_minis >;
  friend class UniversalDataLogger< ht_neuron_minis >;
  ...
  struct Buffers_
  {
    Buffers_( ht_neuron_minis& );
    Buffers_( const Buffers_&, ht_neuron_minis& );

    UniversalDataLogger< ht_neuron_minis > logger_;
   ...
   *
   * Receives V_m as argument since it is called from ht_neuron_minis_dyamics
  ...
  static RecordablesMap< ht_neuron_minis > recordablesMap_;
...
inline port
ht_neuron_minis::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
ht_neuron_minis::handles_test_event( SpikeEvent&, rport receptor_type )
{
...
inline port
ht_neuron_minis::handles_test_event( CurrentEvent&, rport receptor_type )
{
...
inline port
ht_neuron_minis::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
    throw UnknownReceptorType( receptor_type, get_name() );

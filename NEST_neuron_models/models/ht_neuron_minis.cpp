...
namespace nest
{
RecordablesMap< ht_neuron_minis > ht_neuron_minis::recordablesMap_;
template <>
void
RecordablesMap< ht_neuron_minis >::create()
{
  insert_( names::V_m, &ht_neuron_minis::get_y_elem_< ht_neuron_minis::State_::V_M > );
  insert_( names::theta, &ht_neuron_minis::get_y_elem_< ht_neuron_minis::State_::THETA > );
  insert_(
    names::g_AMPA, &ht_neuron_minis::get_y_elem_< ht_neuron_minis::State_::G_AMPA > );
  insert_( names::g_NMDA, &ht_neuron_minis::get_g_NMDA_ );
  insert_(
    names::g_GABA_A, &ht_neuron_minis::get_y_elem_< ht_neuron_minis::State_::G_GABA_A > );
  insert_(
    names::g_GABA_B, &ht_neuron_minis::get_y_elem_< ht_neuron_minis::State_::G_GABA_B > );
  insert_( names::I_NaP, &ht_neuron_minis::get_I_NaP_ );
  insert_( names::I_KNa, &ht_neuron_minis::get_I_KNa_ );
  insert_( names::I_T, &ht_neuron_minis::get_I_T_ );
  insert_( names::I_h, &ht_neuron_minis::get_I_h_ );
}
...
extern "C" inline int
ht_neuron_minis_dynamics( double, const double y[], double f[], void* pnode )
{
  // shorthand
  typedef nest::ht_neuron_minis::State_ S;

  // get access to node so we can almost work as in a member class
  assert( pnode );
  nest::ht_neuron_minis& node = *( reinterpret_cast< nest::ht_neuron_minis* >( pnode ) );
...
inline double
nest::ht_neuron_minis::m_eq_h_( double V ) const
{
...
inline double
nest::ht_neuron_minis::h_eq_T_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( ( V + 83.0 ) / 4 ) );
}

inline double
nest::ht_neuron_minis::m_eq_T_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( -( V + 59.0 ) / 6.2 ) );
}

inline double
nest::ht_neuron_minis::D_eq_KNa_( double V ) const
{
...
inline double
nest::ht_neuron_minis::m_eq_NMDA_( double V ) const
{
  return 1.0 / ( 1.0 + std::exp( -P_.S_act_NMDA * ( V - P_.V_act_NMDA ) ) );
}

inline double
nest::ht_neuron_minis::m_NMDA_( double V,
  double m_eq,
  double m_fast,
  double m_slow ) const
{
...
inline double
nest::ht_neuron_minis::get_g_NMDA_() const
{
...
nest::ht_neuron_minis::Parameters_::Parameters_()
  : E_Na( 30.0 ) // mV
  , E_K( -90.0 ) // mV
  , g_NaL( 0.2 )
  , g_KL( 1.0 )
...
nest::ht_neuron_minis::State_::State_( const ht_neuron_minis& node, const Parameters_& p )
  : ref_steps_( 0 )
  , I_NaP_( 0.0 )
  , I_KNa_( 0.0 )
  , I_T_( 0.0 )
...
nest::ht_neuron_minis::State_::State_( const State_& s )
...
nest::ht_neuron_minis::State_& nest::ht_neuron_minis::State_::operator=( const State_& s )
{
  if ( this == &s )
  {
    return *this;
...
nest::ht_neuron_minis::State_::~State_()
{
}
...
void
nest::ht_neuron_minis::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::ht_neuron_minis::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::ht_neuron_minis::State_::get( DictionaryDatum& d ) const
{
...
void
nest::ht_neuron_minis::State_::set( const DictionaryDatum& d, const ht_neuron_minis& node )
{
...
nest::ht_neuron_minis::Buffers_::Buffers_( ht_neuron_minis& n )
  : logger_( n )
...
nest::ht_neuron_minis::Buffers_::Buffers_( const Buffers_&, ht_neuron_minis& n )
  : logger_( n )
  , spike_inputs_( std::vector< RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) )
  , s_( 0 )
  , c_( 0 )
...
nest::ht_neuron_minis::ht_neuron_minis()
  : Archiving_Node()
  , P_()
  , S_( *this, P_ )
  , B_( *this )
...
nest::ht_neuron_minis::ht_neuron_minis( const ht_neuron_minis& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::ht_neuron_minis::init_state_( const Node& proto )
{
  const ht_neuron_minis& pr = downcast< ht_neuron_minis >( proto );
  S_ = pr.S_;
}

void
nest::ht_neuron_minis::init_buffers_()
{
  // Reset spike buffers.
  ...
  B_.sys_.function = ht_neuron_minis_dynamics;
  B_.sys_.jacobian = 0;
...
double
nest::ht_neuron_minis::get_synapse_constant( double tau_1,
  double tau_2,
  double g_peak )
{
...
void
nest::ht_neuron_minis::calibrate()
{
...
void
nest::ht_neuron_minis::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
void
nest::ht_neuron_minis::set_status( const DictionaryDatum& d )
{
...
void
ht_neuron_minis::update( Time const& origin, const long from, const long to )
{
  assert(
...
void
nest::ht_neuron_minis::handle( SpikeEvent& e )
{
...
void
nest::ht_neuron_minis::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::ht_neuron_minis::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
}
